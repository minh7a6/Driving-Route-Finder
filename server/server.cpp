...
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (ack == "A") {
            ...
            port.writeline("W ");
            port.writeline(to_string(p[route.top()].lat));
            port.writeline(" ");
            port.writeline(to_string(p[route.top()].lon));
            port.writeline("\n");
    ...
    while(true) {
        ...
        while (!timeout) {
            ...
            if (request[0] == "R") {
                ...
                startLat = stoi(request[1]);
                startLon = stoi(request[2]);
                endLat = stoi(request[3]);
                endLon = stoi(request[4]);
